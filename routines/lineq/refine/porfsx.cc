...
void dporfsx(const v8::FunctionCallbackInfo<v8::Value>& info) {
	...
	lapack_int ldb = info[10].As<v8::Integer>()->Value();
	double *x = reinterpret_cast<double*>(GET_CONTENTS(info[11].As<v8::Float64Array>()));
	lapack_int ldx = info[12].As<v8::Integer>()->Value();
	...
	lapack_int n_err_bnds = info[15].As<v8::Integer>()->Value();
	...
	lapack_int nparams = info[18].As<v8::Integer>()->Value();
